...
const statusConfig: Record<string, { label: string; color: string; bgColor: string }> = {
  ...
  paused: { label: 'Pausado', color: 'text-gray-700', bgColor: 'bg-gray-100' },
  ...
  return (
    <Card className="overflow-hidden">
      <div className="flex flex-col sm:flex-row">
        {/* Image */}
        <div className="relative h-40 w-full flex-shrink-0 bg-gray-100 sm:h-auto sm:w-48">
          {vehicle.imageUrl ? (
            <img
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-gray-600">{vehicles.length} publicaciones en total</p>
        </div>
        <Button asChild>
          <Link href="/vender">
            <Plus className="mr-2 h-4 w-4" />
          ...
          {tabs.map(tab => (
            ...
            >
              ...
              {statusCounts[tab.value] > 0 && (
                <span className="ml-1.5 text-xs text-gray-500">({statusCounts[tab.value]})</span>
              )}
            </button>
          ))}
        </div>
