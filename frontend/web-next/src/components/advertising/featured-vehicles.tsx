...
function FeaturedVehicleCard({
  ...
  return (
    <Link href={vehicleHref} onClick={handleClick} className="group block">
      <Card className="overflow-hidden border-0 shadow-md transition-all hover:shadow-xl hover:-translate-y-0.5">
        ...
        <div className="bg-muted relative" style={{ aspectRatio: '4/3' }}>
          {vehicle.imageUrl ? (
            ...
            <div className="bg-slate-100 dark:bg-slate-800 flex h-full items-center justify-center">
          ...
          <span className="absolute bottom-2 right-2 rounded-full bg-black/50 px-2 py-0.5 text-[10px] text-white/80 backdrop-blur-sm">
            Patrocinado
          </span>
        </div>
...
export default function FeaturedVehicles({
  ...
  const accentColor = placementType === 'PremiumSpot'
  ...
  const viewAllHref = placementType === 'PremiumSpot'
    ? '/vehiculos?sortBy=price_desc'
    : '/vehiculos?sortBy=newest';
