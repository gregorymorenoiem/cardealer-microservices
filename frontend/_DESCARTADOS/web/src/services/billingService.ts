...
// Payment gateways supported by PaymentService
...
export const availableGateways: PaymentGatewayInfo[] = [
  {
    ...
    description: 'Tarjetas de crédito y débito locales RD',
    recommended: true,
    local: true,
  },
  {
    id: 'CardNET',
    name: 'CardNET',
    description: 'Red de pagos bancarios dominicanos',
    local: true,
  },
  {
    id: 'PixelPay',
    name: 'PixelPay',
    description: 'Pasarela fintech con bajas comisiones',
    local: true,
  },
  {
    id: 'Fygaro',
    name: 'Fygaro',
    description: 'Agregador de pagos múltiples métodos',
    local: true,
  },
  {
    id: 'PayPal',
    name: 'PayPal',
    description: 'Pagos internacionales con tarjetas extranjeras',
    local: false,
  },
];
...
// ============================================================================
// PAYMENT METHODS API
// Now calls PaymentService (multi-provider) instead of BillingService
// ============================================================================
...
export const paymentMethodsApi = {
  ...
  getAll: async (): Promise<PaymentMethodInfo[]> => {
    ...
    try {
      // Call PaymentService directly - uses JWT auth, no dealerId needed
      const response = await api.get<{ methods: PaymentMethodInfo[]; total: number }>(
        '/payment-methods'
      );
  ...
  getAvailableGateways: async (): Promise<PaymentGatewayInfo[]> => {
    // For now, return static list. In future could call /payment-methods/gateways
    return availableGateways;
  },

  // Add a new payment method with gateway selection
  add: async (data: AddPaymentMethodRequest): Promise<PaymentMethodInfo> => {
    ...
    // Call PaymentService - it will tokenize the card with the selected gateway
    const response = await api.post<PaymentMethodInfo>('/payment-methods', data);
    return response.data;
  },
  ...
  setDefault: async (paymentMethodId: string): Promise<void> => {
    ...
    await api.post(`/payment-methods/${paymentMethodId}/default`);
  },
