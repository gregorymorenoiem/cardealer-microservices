/**
 * MapViewPage - Professional real estate map view
 * Shows properties and agencies on an interactive map
 */
...
import { mockPropertyDealers } from '@/data/mockDealers';
import { 
  FiX, FiPhone, FiMapPin, FiFilter, FiChevronLeft, FiCheckCircle,
  FiMessageCircle, FiCopy, FiExternalLink, FiStar, FiShare2, FiNavigation
} from 'react-icons/fi';
...
export default function MapViewPage() {
  ...
  const getDistance = useCallback((dealer: typeof mockPropertyDealers[0]) => {
    const R = 6371;
  ...
  const filteredDealers = useMemo(() => {
    return mockPropertyDealers.filter(dealer => {
  ...
  const handleShare = useCallback((dealer: typeof mockPropertyDealers[0], method: 'copy' | 'whatsapp') => {
    const url = `${window.location.origin}/properties/map?dealer=${dealer.id}`;
      ...
      <MainLayout>
        <div className="h-screen flex items-center justify-center bg-gray-50">
          <div className="text-center">
            ...
            <p className="text-gray-600 font-medium">Cargando mapa de inmobiliarias...</p>
          </div>
        </div>
      </MainLayout>
    );
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-base font-bold text-gray-900">
                  {filteredDealers.length} inmobiliarias
                </h1>
              </div>
            </div>
                    ...
                    <div className="flex items-center justify-between py-2">
                      ...
                      <button
                        ...
                        className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
                          filters.verified ? 'bg-emerald-600' : 'bg-gray-300'
                        }`}
                      >
                        <span
                ...
                <div className="p-3">
                  <div className="flex items-center justify-between mb-2">
                    <h4 className="text-sm font-semibold text-gray-700">
                      {selectedDealerData.activeListings} propiedades
                    </h4>
                  ...
                  <div className="grid grid-cols-3 gap-2 mb-3">
                    {selectedDealerData.featuredListings.slice(imageStartIndex, imageStartIndex + 3).map((listing) => (
                      <div 
                        ...
                        onDoubleClick={() => navigate(`/properties/${listing.id}`)}
                      >
                  ...
                  {selectedDealerData.activeListings > 3 && (
                    ...
                    >
                      Ver todas ({selectedDealerData.activeListings})
