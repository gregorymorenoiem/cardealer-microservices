  ...
  const FilterContent = () => (
    <div className="space-y-6">
      ...
      <div>
        ...
        >
          <option value="year-desc">{t('vehicles:sort.yearDesc')}</option>
          <option value="year-asc">{t('vehicles:sort.yearAsc')}</option>
          <option value="price-asc">{t('vehicles:sort.priceAsc')}</option>
          <option value="price-desc">{t('vehicles:sort.priceDesc')}</option>
          <option value="mileage-asc">{t('vehicles:sort.mileageAsc')}</option>
          <option value="mileage-desc">{t('vehicles:sort.mileageDesc')}</option>
        </select>
      </div>
      ...
      <div>
        ...
        <input
          ...
          placeholder={t('vehicles:filters.enterModelName')}
          className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent"
        />
      </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          >
            <option value="">{t('vehicles:filters.minYear')}</option>
            {years.map((year) => (
          ...
          <select
            ...
            className="px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent text-sm"
          >
            <option value="">{t('vehicles:filters.maxYear')}</option>
            {years.map((year) => (
              <option key={year} value={year}>{year}</option>
            ))}
          </select>
      ...
      <div>
        <label className="block text-sm font-semibold text-gray-900 mb-3">
          {t('vehicles:filters.mileageRange')}
        </label>
        <div className="grid grid-cols-2 gap-3">
          <input
            type="number"
      ...
      <div>
        ...
        <select
          ...
          className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent"
        >
          <option value="">{t('vehicles:filters.allTransmissions')}</option>
          {transmissions.map((trans) => (
            <option key={trans} value={trans}>{t(`vehicles:transmission.${trans.toLowerCase()}`)}</option>
          ))}
        </select>
      </div>
      ...
      <div>
        ...
        <select
          ...
          className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent"
        >
          <option value="">{t('vehicles:filters.allFuelTypes')}</option>
          {fuelTypes.map((fuel) => (
            <option key={fuel} value={fuel}>{t(`vehicles:fuelType.${fuel.toLowerCase().replace(/-/g, '')}`)}</option>
          ))}
        </select>
      </div>
      ...
      <div>
        ...
        <select
          ...
          className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent"
        >
          <option value="">{t('vehicles:filters.allBodyTypes')}</option>
          {bodyTypes.map((body) => (
            <option key={body} value={body}>{t(`vehicles:bodyType.${body.toLowerCase()}`)}</option>
          ))}
        </select>
      </div>
      ...
      <div>
        ...
        <select
          ...
          className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent"
        >
          <option value="">{t('vehicles:filters.allConditions')}</option>
          {conditions.map((cond) => (
            <option key={cond} value={cond}>{t(`vehicles:conditions.${cond.toLowerCase().replace(/ /g, '')}`)}</option>
          ))}
        </select>
      </div>
      ...
      {activeFilterCount > 0 && (
        ...
        >
          {t('vehicles:filters.clearAllCount', { count: activeFilterCount })}
        </button>
      )}
    </div>
  );
  ...
  return (
    <>
      ...
      <div className="lg:hidden mb-4">
        ...
        >
          <FiSliders size={20} />
          <span className="font-medium">{t('vehicles:filters.title')} {activeFilterCount > 0 && `(${activeFilterCount})`}</span>
        </button>
      </div>
      ...
      <div className="hidden lg:block bg-white rounded-xl shadow-card p-6 sticky top-24">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-xl font-bold text-gray-900">{t('vehicles:filters.title')}</h2>
          {activeFilterCount > 0 && (
