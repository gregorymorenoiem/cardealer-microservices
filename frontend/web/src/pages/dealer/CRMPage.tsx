          ...
          {showMenu && (
            <>
              ...
              <div className="absolute right-0 top-6 bg-white rounded-lg shadow-lg border border-gray-200 py-1 z-10 min-w-[120px]">
                <button
                  onClick={() => { onMove(deal.id, 'prev'); setShowMenu(false); }}
                ...
                <button
                  onClick={() => { onMove(deal.id, 'next'); setShowMenu(false); }}
                  className="w-full px-3 py-1.5 text-left text-sm text-gray-700 hover:bg-gray-50 flex items-center gap-2"
                >
      ...
      <div className="flex items-center justify-between text-xs">
        <div className="flex items-center gap-1 text-gray-500">
          ...
          {deal.expectedCloseDate 
            ? new Date(deal.expectedCloseDate).toLocaleDateString('es-MX', { day: 'numeric', month: 'short' })
            : 'Sin fecha'
          }
        </div>
        <span 
...
const CRMPage = () => {
      ...
      <div className="min-h-[60vh] flex items-center justify-center">
        <div className="text-center max-w-md">
          ...
          <p className="text-gray-600 mb-6">
            El módulo de CRM está disponible a partir del plan <span className="font-medium text-blue-600">PRO</span>.
            Actualiza tu plan para acceder a gestión de leads, pipeline de ventas y más.
          </p>
          <Link
            to="/dealer/plans"
  ...
  const pipelineStats = useMemo(() => {
    ...
    return {
      ...
      avgDealSize: pipelineDeals.length > 0 
        ? pipelineDeals.reduce((sum, d) => sum + d.value, 0) / pipelineDeals.length 
        : 0,
      conversionRate: pipelineDeals.length > 0
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="flex gap-4 overflow-hidden">
          {[1, 2, 3, 4, 5].map(i => (
            <div key={i} className="w-72 h-96 bg-gray-200 rounded-lg animate-pulse flex-shrink-0" />
          ))}
        </div>
      </div>
    );
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-gray-600 mt-1">
            Gestiona tu pipeline de ventas y leads
          </p>
        </div>
        <div className="flex items-center gap-3">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-white rounded-lg border border-gray-200 p-4">
          ...
          <p className="text-xs text-gray-500">{formatValue(pipelineStats.totalValue)} en pipeline</p>
        </div>
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-4">
          ...
          <p className="text-2xl font-bold text-gray-900 mt-1">{formatValue(pipelineStats.openValue)}</p>
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-4">
          ...
          <p className="text-2xl font-bold text-gray-900 mt-1">{formatValue(pipelineStats.wonValue)}</p>
