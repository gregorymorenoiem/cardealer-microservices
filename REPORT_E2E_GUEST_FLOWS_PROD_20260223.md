...
| **BUG-S001** | NotificationService | KYC approval email                        | RabbitMQ `notificationservice.kyc.status_changed` sin consumer; excepciones en email causaban NACK y acumulación en DLX | 2 causas: (1) `KYCStatusChangedNotificationConsumer.HandleKYCStatusAsync()` lanzaba exception en error de email → NACK → DLQ; (2) Consumer nunca registrado como `HostedService` | Implementar fire-and-forget pattern: cambiar exception handling (log sin rethrow), registrar consumer en DI            | `a51691a4` |
...
BUGS FIXED:   5/5 ✅ (BUG-D003, D004, D005, D006, S001 — todos corregidos)
COMMITS:      3 (5cbab9ad, dda5b06c, a51691a4)
```
...
1. **BUG-S001 Verification**: ✅ Implementado — Realizar test E2E de KYC approval para verificar que emails se envían (no al DLQ). Ver [BUG_S001_FIX_SUMMARY.md](BUG_S001_FIX_SUMMARY.md) para instrucciones de test manual.
...
4. **isVerified en JWT**: Considerar incluir `isVerified` como claim en el JWT del AuthService para evitar round-trips al UserService cuando solo se necesita el estado de verificación.
5. **DLQ cleanup (post-fix)**: Opcionalmente limpiar mensajes viejos del DLQ de KYC:
   ```bash
...
_Generado automáticamente por GitHub Copilot (Claude Sonnet 4.6) — Auditoría E2E OKLA 2026-02-23_
