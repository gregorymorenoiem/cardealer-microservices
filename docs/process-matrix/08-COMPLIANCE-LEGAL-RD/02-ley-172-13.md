# 🔒 Ley 172-13 - Protección de Datos Personales - Matriz de Procesos

> **Marco Legal:** Ley 172-13 sobre Protección de Datos de Carácter Personal  
> **Regulador:** Superintendencia de Bancos / Procuraduría General  
> **Última actualización:** Enero 25, 2026  
> **Estado:** 🟢 ACTIVO (Obligatorio)  
> **Estado de Implementación:** 🟡 60% Backend | 🟡 40% UI

---

## ⚠️ AUDITORÍA DE ACCESO UI (Enero 25, 2026)

| Proceso                         | Backend        | UI Access       | Observación       |
| ------------------------------- | -------------- | --------------- | ----------------- |
| GDPR-CONSENT-001 Consentimiento | ✅ AuthService | ✅ RegisterPage | CheckBox términos |
| GDPR-ACCESS-001 Ver mis datos   | 🟡 Parcial     | ✅ ProfilePage  | Datos básicos     |
| GDPR-DELETE-001 Eliminar cuenta | 🟡 Parcial     | 🟡 SettingsPage | Solicitud manual  |
| GDPR-EXPORT-001 Exportar datos  | 🔴 Pendiente   | 🔴 Falta        | Sin descarga JSON |

### Rutas UI Existentes ✅

- `/register` → Consentimiento en registro
- `/settings/privacy` → Opciones de privacidad básicas

### Rutas UI Faltantes 🔴

- `/settings/privacy/export` → Exportar todos mis datos
- `/settings/privacy/delete` → Solicitar eliminación completa
- `/privacy-center` → Centro de privacidad completo

**Verificación Backend:** Funcionalidades parciales en UserService 🟡

---

## 📊 Resumen de Implementación

| Componente                           | Total | Implementado | Pendiente | Estado         |
| ------------------------------------ | ----- | ------------ | --------- | -------------- |
| **GDPR-CONSENT-\*** (Consentimiento) | 4     | 4            | 0         | ✅ Completo    |
| **GDPR-ACCESS-\*** (Acceso Datos)    | 3     | 2            | 1         | 🟡 Parcial     |
| **GDPR-DELETE-\*** (Eliminación)     | 3     | 2            | 1         | 🟡 Parcial     |
| **GDPR-EXPORT-\*** (Portabilidad)    | 3     | 0            | 3         | 🔴 Pendiente   |
| **GDPR-BREACH-\*** (Brechas)         | 3     | 2            | 1         | 🟡 Parcial     |
| **Tests**                            | 15    | 8            | 7         | 🟡 Parcial     |
| **TOTAL**                            | 31    | 18           | 13        | 🟡 60% Backend |

---

## 1. Información General

### 1.1 Descripción

La Ley 172-13 establece el régimen jurídico para la protección de datos personales en República Dominicana. Similar al GDPR europeo, otorga derechos a los titulares de datos y obligaciones a quienes los procesan.

### 1.2 Definiciones Clave

| Término              | Definición                                          |
| -------------------- | --------------------------------------------------- |
| **Datos personales** | Información de persona identificada o identificable |
| **Titular**          | Persona física dueña de los datos                   |
| **Responsable**      | Quien decide finalidad del tratamiento (OKLA)       |
| **Encargado**        | Quien trata datos por cuenta del responsable        |
| **Tratamiento**      | Cualquier operación sobre datos personales          |
| **Consentimiento**   | Manifestación libre, específica e informada         |

### 1.3 Datos Sensibles (Requieren Consentimiento Expreso)

| Tipo                    | Ejemplos                       |
| ----------------------- | ------------------------------ |
| Origen racial/étnico    | Nacionalidad, etnia            |
| Opiniones políticas     | Afiliación partidaria          |
| Convicciones religiosas | Religión, creencias            |
| Afiliación sindical     | Membresía en sindicatos        |
| Datos de salud          | Condiciones médicas            |
| Vida sexual             | Orientación sexual             |
| Datos biométricos       | Huellas, reconocimiento facial |

---

## 2. Derechos del Titular (Derechos ARCO)

### 2.1 Derecho de Acceso

| Campo                  | Valor                                   |
| ---------------------- | --------------------------------------- |
| **Descripción**        | Conocer qué datos personales tiene OKLA |
| **Plazo de respuesta** | 10 días hábiles                         |
| **Formato**            | Digital (JSON/PDF)                      |
| **Costo**              | Gratis                                  |

### 2.2 Derecho de Rectificación

| Campo                  | Valor                                  |
| ---------------------- | -------------------------------------- |
| **Descripción**        | Corregir datos inexactos o incompletos |
| **Plazo de respuesta** | 10 días hábiles                        |
| **Evidencia**          | Documento que soporte la corrección    |

### 2.3 Derecho de Cancelación (Eliminación)

| Campo                  | Valor                                       |
| ---------------------- | ------------------------------------------- |
| **Descripción**        | Eliminar datos cuando ya no sean necesarios |
| **Plazo de respuesta** | 10 días hábiles                             |
| **Excepciones**        | Obligaciones legales, ejercicio de derechos |

### 2.4 Derecho de Oposición

| Campo                  | Valor                                |
| ---------------------- | ------------------------------------ |
| **Descripción**        | Oponerse al tratamiento de sus datos |
| **Plazo de respuesta** | 10 días hábiles                      |
| **Resultado**          | Cese del tratamiento                 |

---

## 3. Endpoints API de Privacidad

### 3.1 PrivacyController

| Método | Endpoint                     | Descripción                 | Auth | Roles |
| ------ | ---------------------------- | --------------------------- | ---- | ----- |
| `GET`  | `/api/privacy/my-data`       | Exportar mis datos (Acceso) | ✅   | User  |
| `POST` | `/api/privacy/rectification` | Solicitar rectificación     | ✅   | User  |
| `POST` | `/api/privacy/deletion`      | Solicitar eliminación       | ✅   | User  |
| `POST` | `/api/privacy/opposition`    | Oponerse a tratamiento      | ✅   | User  |
| `GET`  | `/api/privacy/consents`      | Ver mis consentimientos     | ✅   | User  |
| `PUT`  | `/api/privacy/consents/{id}` | Actualizar consentimiento   | ✅   | User  |
| `GET`  | `/api/privacy/data-usage`    | Ver uso de mis datos        | ✅   | User  |

### 3.2 AdminPrivacyController

| Método | Endpoint                           | Descripción             | Auth | Roles   |
| ------ | ---------------------------------- | ----------------------- | ---- | ------- |
| `GET`  | `/api/admin/privacy/requests`      | Listar solicitudes ARCO | ✅   | Privacy |
| `PUT`  | `/api/admin/privacy/requests/{id}` | Procesar solicitud      | ✅   | Privacy |
| `GET`  | `/api/admin/privacy/data-map`      | Mapa de datos           | ✅   | Privacy |
| `GET`  | `/api/admin/privacy/breach-log`    | Log de brechas          | ✅   | Privacy |

---

## 4. Procesos Detallados

### 4.1 PRIV-001: Exportar Datos Personales (Acceso)

| Campo       | Valor                     |
| ----------- | ------------------------- |
| **ID**      | PRIV-001                  |
| **Nombre**  | Exportar Datos Personales |
| **Actor**   | Usuario (Titular)         |
| **Trigger** | GET /api/privacy/my-data  |

#### Flujo del Proceso

| Paso | Acción                        | Sistema             | Validación           |
| ---- | ----------------------------- | ------------------- | -------------------- |
| 1    | Usuario accede a "Privacidad" | Frontend            | Settings             |
| 2    | Click "Descargar mis datos"   | Frontend            | Confirmar            |
| 3    | Verificar identidad           | AuthService         | 2FA si habilitado    |
| 4    | Recopilar datos de User       | UserService         | Perfil completo      |
| 5    | Recopilar vehículos           | VehiclesSaleService | Listados del usuario |
| 6    | Recopilar favoritos           | FavoritesService    | Lista de favoritos   |
| 7    | Recopilar búsquedas           | SearchService       | Historial            |
| 8    | Recopilar transacciones       | BillingService      | Pagos realizados     |
| 9    | Recopilar mensajes            | ChatService         | Conversaciones       |
| 10   | Compilar en JSON/ZIP          | PrivacyService      | Formato portable     |
| 11   | Generar link de descarga      | MediaService        | Expira en 24h        |
| 12   | Notificar usuario             | NotificationService | Email con link       |
| 13   | Log para auditoría            | AuditService        | Registro de acceso   |

#### Response (Ejemplo de Datos Exportados)

```json
{
  "exportDate": "2026-01-21T10:30:00Z",
  "user": {
    "id": "uuid",
    "email": "juan@email.com",
    "fullName": "Juan Pérez",
    "phone": "+18295550100",
    "createdAt": "2025-03-15T08:00:00Z",
    "lastLogin": "2026-01-20T18:45:00Z"
  },
  "profile": {
    "avatar": "https://...",
    "bio": "Comprador de vehículos...",
    "location": "Santo Domingo"
  },
  "vehicles": [
    {
      "id": "uuid",
      "title": "Toyota Corolla 2020",
      "status": "Active",
      "createdAt": "2025-06-10"
    }
  ],
  "favorites": [{ "vehicleId": "uuid", "addedAt": "2025-12-01" }],
  "searchHistory": [{ "query": "SUV Toyota", "date": "2026-01-15" }],
  "transactions": [{ "id": "uuid", "amount": 12900, "date": "2026-01-01" }],
  "consents": [{ "type": "marketing", "granted": true, "date": "2025-03-15" }]
}
```

---

### 4.2 PRIV-002: Solicitud de Eliminación

| Campo       | Valor                          |
| ----------- | ------------------------------ |
| **ID**      | PRIV-002                       |
| **Nombre**  | Solicitar Eliminación de Datos |
| **Actor**   | Usuario (Titular)              |
| **Trigger** | POST /api/privacy/deletion     |

#### Flujo del Proceso

| Paso | Acción                          | Sistema             | Validación            |
| ---- | ------------------------------- | ------------------- | --------------------- |
| 1    | Usuario solicita eliminación    | Frontend            | Modal de confirmación |
| 2    | Verificar identidad             | AuthService         | Password + 2FA        |
| 3    | Verificar no tiene obligaciones | BillingService      | Sin deudas            |
| 4    | Verificar no tiene disputas     | EscrowService       | Sin casos abiertos    |
| 5    | Crear solicitud                 | PrivacyService      | Status = Pending      |
| 6    | Notificar equipo privacy        | NotificationService | Ticket creado         |
| 7    | Periodo de gracia 7 días        | SchedulerService    | Cancelable            |
| 8    | Si no cancela                   | PrivacyService      | Proceder              |
| 9    | Anonimizar datos                | PrivacyService      | Reemplazar PII        |
| 10   | Eliminar datos no requeridos    | PrivacyService      | Según política        |
| 11   | Mantener datos legales          | Database            | 10 años (Ley 155-17)  |
| 12   | Eliminar cuenta                 | UserService         | Soft delete           |
| 13   | Enviar confirmación             | NotificationService | Email final           |
| 14   | Log para auditoría              | AuditService        | Registro inmutable    |

#### Datos que NO se eliminan (Obligación Legal)

| Dato           | Razón               | Retención  |
| -------------- | ------------------- | ---------- |
| Transacciones  | Ley 155-17 (AML)    | 10 años    |
| Facturas       | Ley 253-12 (Fiscal) | 10 años    |
| Logs de acceso | Seguridad           | 2 años     |
| ROS enviados   | Confidencial UAF    | Permanente |

---

### 4.3 PRIV-003: Gestión de Consentimientos

| Campo       | Valor                          |
| ----------- | ------------------------------ |
| **ID**      | PRIV-003                       |
| **Nombre**  | Gestionar Consentimientos      |
| **Actor**   | Usuario                        |
| **Trigger** | PUT /api/privacy/consents/{id} |

#### Tipos de Consentimiento

| Tipo              | Obligatorio | Default | Descripción                    |
| ----------------- | ----------- | ------- | ------------------------------ |
| `terms`           | ✅          | ✅      | Términos de servicio           |
| `privacy`         | ✅          | ✅      | Política de privacidad         |
| `marketing_email` | ❌          | ❌      | Emails promocionales           |
| `marketing_sms`   | ❌          | ❌      | SMS promocionales              |
| `marketing_push`  | ❌          | ❌      | Push notifications             |
| `analytics`       | ❌          | ✅      | Analytics de uso               |
| `personalization` | ❌          | ✅      | Recomendaciones personalizadas |
| `third_party`     | ❌          | ❌      | Compartir con terceros         |

#### Flujo del Proceso

| Paso | Acción                          | Sistema             | Validación         |
| ---- | ------------------------------- | ------------------- | ------------------ |
| 1    | Usuario accede a Privacy Center | Frontend            | /settings/privacy  |
| 2    | Ver consentimientos actuales    | PrivacyService      | Lista con toggles  |
| 3    | Cambiar toggle                  | Frontend            | On/Off             |
| 4    | Confirmar cambio                | Frontend            | Modal si desactiva |
| 5    | Guardar preferencia             | Database            | ConsentHistory     |
| 6    | Propagar a servicios            | RabbitMQ            | consent.updated    |
| 7    | Si marketing desactivado        | NotificationService | Eliminar de listas |
| 8    | Si analytics desactivado        | AnalyticsService    | No trackear        |
| 9    | Log de cambio                   | AuditService        | Con timestamp      |

---

### 4.4 PRIV-004: Notificación de Brecha de Datos

| Campo       | Valor                       |
| ----------- | --------------------------- |
| **ID**      | PRIV-004                    |
| **Nombre**  | Manejar Brecha de Seguridad |
| **Actor**   | Sistema/Admin               |
| **Trigger** | Detección de incidente      |

#### Flujo del Proceso

| Paso | Acción                      | Sistema             | Validación              |
| ---- | --------------------------- | ------------------- | ----------------------- |
| 1    | Detectar brecha             | SecurityService     | Alerta                  |
| 2    | Evaluar severidad           | IncidentService     | Bajo/Medio/Alto/Crítico |
| 3    | Contener incidente          | SecurityService     | Bloquear acceso         |
| 4    | Identificar datos afectados | PrivacyService      | Cuáles usuarios         |
| 5    | Documentar incidente        | IncidentService     | Detalle completo        |
| 6    | Si alto riesgo              | NotificationService | Notificar titulares     |
| 7    | Notificar autoridades       | ExternalService     | 72 horas max            |
| 8    | Implementar remediación     | DevOps              | Parche/Fix              |
| 9    | Post-mortem                 | IncidentService     | Lecciones aprendidas    |
| 10   | Actualizar políticas        | PrivacyService      | Si necesario            |

#### Template de Notificación a Usuarios

```
Estimado/a {{userName}},

Le informamos que hemos detectado un incidente de seguridad que
podría haber afectado algunos de sus datos personales.

¿Qué pasó?
{{incidentDescription}}

¿Qué datos se vieron afectados?
{{affectedDataTypes}}

¿Qué estamos haciendo?
{{remediationSteps}}

¿Qué puede hacer usted?
- Cambiar su contraseña
- Activar autenticación de dos factores
- Revisar actividad reciente de su cuenta

Para más información, contacte a nuestro equipo de privacidad:
privacy@okla.com.do

Lamentamos los inconvenientes causados.

Atentamente,
Equipo de Seguridad OKLA
```

---

## 5. Aviso de Privacidad

### 5.1 Contenido Obligatorio

| Sección            | Contenido                                     |
| ------------------ | --------------------------------------------- |
| **Identidad**      | OKLA Technologies SRL, RNC 131-12345-6        |
| **Contacto DPO**   | privacy@okla.com.do                           |
| **Finalidades**    | Prestación del servicio, marketing, analytics |
| **Base legal**     | Consentimiento, contrato, obligación legal    |
| **Destinatarios**  | Stripe, AWS, proveedores de servicio          |
| **Transferencias** | Estados Unidos (Stripe, AWS)                  |
| **Derechos**       | ARCO + queja ante autoridad                   |
| **Retención**      | Según tipo de dato                            |

### 5.2 Momentos de Presentación

| Momento                | Formato                       |
| ---------------------- | ----------------------------- |
| Registro               | Link en formulario + checkbox |
| Login (si actualizado) | Modal informativo             |
| Checkout               | Resumen de datos recopilados  |
| Cookies                | Banner con opciones           |

---

## 6. Entidades

### 6.1 PrivacyRequest (Solicitud ARCO)

```csharp
public class PrivacyRequest
{
    public Guid Id { get; set; }
    public Guid UserId { get; set; }
    public PrivacyRequestType Type { get; set; }
    public PrivacyRequestStatus Status { get; set; }
    public string Details { get; set; }
    public DateTime RequestedAt { get; set; }
    public DateTime? ProcessedAt { get; set; }
    public string? ProcessedBy { get; set; }
    public string? Response { get; set; }
    public DateTime Deadline { get; set; }  // +10 días hábiles
}

public enum PrivacyRequestType
{
    Access,
    Rectification,
    Deletion,
    Opposition,
    Portability
}

public enum PrivacyRequestStatus
{
    Pending,
    InProgress,
    Completed,
    Rejected
}
```

### 6.2 Consent (Consentimiento)

```csharp
public class Consent
{
    public Guid Id { get; set; }
    public Guid UserId { get; set; }
    public string ConsentType { get; set; }
    public bool Granted { get; set; }
    public DateTime GrantedAt { get; set; }
    public DateTime? RevokedAt { get; set; }
    public string? Source { get; set; }  // registration, settings, campaign
    public string IpAddress { get; set; }
    public string UserAgent { get; set; }
}
```

---

## 7. Eventos RabbitMQ

| Evento                      | Exchange         | Payload                       |
| --------------------------- | ---------------- | ----------------------------- |
| `privacy.request.created`   | `privacy.events` | `{ requestId, type, userId }` |
| `privacy.request.completed` | `privacy.events` | `{ requestId }`               |
| `privacy.consent.updated`   | `privacy.events` | `{ userId, type, granted }`   |
| `privacy.data.exported`     | `privacy.events` | `{ userId, format }`          |
| `privacy.data.deleted`      | `privacy.events` | `{ userId }`                  |
| `privacy.breach.detected`   | `privacy.events` | `{ severity, affectedCount }` |

---

## 8. Métricas

```
# Solicitudes ARCO
privacy_requests_total{type="access|rectification|deletion|opposition"}
privacy_requests_pending_total
privacy_requests_completed_total
privacy_requests_response_time_hours

# Consentimientos
privacy_consents_granted_total{type="..."}
privacy_consents_revoked_total{type="..."}
privacy_consent_rate{type="..."}

# Brechas
privacy_breaches_total{severity="low|medium|high|critical"}
privacy_affected_users_total
```

---

## 9. Configuración

```json
{
  "Privacy": {
    "ResponseDeadlineDays": 10,
    "DeletionGracePeriodDays": 7,
    "DataExportFormats": ["json", "csv"],
    "DataExportLinkExpiryHours": 24,
    "BreachNotificationHours": 72,
    "ConsentVersion": "2.0",
    "DPOEmail": "privacy@okla.com.do"
  },
  "DataRetention": {
    "UserProfileYears": 3,
    "TransactionsYears": 10,
    "AccessLogsYears": 2,
    "SearchHistoryDays": 90,
    "DeletedAccountDataDays": 30
  }
}
```

---

## 10. Sanciones por Incumplimiento

| Infracción                     | Sanción                  |
| ------------------------------ | ------------------------ |
| No responder solicitud ARCO    | Multa hasta RD$500,000   |
| Tratamiento sin consentimiento | Multa + indemnización    |
| No notificar brecha            | Multa hasta RD$2,000,000 |
| Transferencia no autorizada    | Multa + suspensión       |

---

## 📚 Referencias

- [Ley 172-13](https://poderjudicial.gob.do/ley-172-13) - Texto completo
- [01-ley-155-17.md](01-ley-155-17.md) - Lavado de activos
- [03-security-2fa.md](../01-AUTENTICACION-SEGURIDAD/03-security-2fa.md) - Seguridad
- [01-auth-service.md](../01-AUTENTICACION-SEGURIDAD/01-auth-service.md) - Autenticación
