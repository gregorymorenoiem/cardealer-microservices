...
public class GetVehicleSpinQueryHandler : IRequestHandler<GetVehicleSpinQuery, SpinGenerationDto?>
{
    private readonly ISpinGenerationRepository _repository;

    public GetVehicleSpinQueryHandler(ISpinGenerationRepository repository)
    {
        _repository = repository;
    }

    public async Task<SpinGenerationDto?> Handle(GetVehicleSpinQuery request, CancellationToken cancellationToken)
    {
        var spin = await _repository.GetByVehicleIdAsync(request.VehicleId, cancellationToken);
