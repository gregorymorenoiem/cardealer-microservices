        ...
        if (!_isRegistered)
        {
            lock (_lock)
            {
                if (!_isRegistered)
                {
                    try
                    {
                        var serviceName = configuration["Service:Name"] ?? "UserService";
                        ...
                        var instance = new ServiceInstance
                        {
                            ...
                            Tags = new List<string> { "user", "consumer", "provider" },
                        ...
                        logger.LogInformation(
                            "✅ UserService registered with Consul: {ServiceName} at {Host}:{Port}",
                    ...
                    {
                        logger.LogError(ex, "❌ Failed to register UserService with Consul");
                    }
                }
            }
        }
