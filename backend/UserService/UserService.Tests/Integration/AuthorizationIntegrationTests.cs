...
namespace UserService.Tests.Integration
{
    public class AuthorizationIntegrationTests : IClassFixture<CustomWebApplicationFactory>
    {
        ...
        private string GenerateJwtToken(string serviceClaim = "UserService", string role = "UserServiceAdmin", int expirationMinutes = 180)
        {
            var claims = new[]
            {
        ...
        public async Task LogError_WithWrongServiceClaim_MayReturnForbiddenOrSucceed()
        {
            ...
            // We're checking it's not successful if policy requires "UserService" claim
            Assert.True(
        ...
        public async Task LogError_WithAdminRole_ReturnsSuccess()
        {
            // Arrange
            var client = _factory.CreateClient();
            var token = GenerateJwtToken(role: "UserServiceAdmin");
            client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", token);
        ...
        public async Task LogError_WithReadRole_ReturnsSuccess()
        {
            // Arrange
            var client = _factory.CreateClient();
            var token = GenerateJwtToken(role: "UserServiceRead");
            client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", token);
