...
namespace UserService.Tests.Controllers
{
    ...
    {
        private ErrorsController CreateControllerWithUser(Mock<IMediator> mediatorMock, string serviceClaim = "UserService", string role = "UserServiceAdmin")
        {
        ...
        [Fact]
        public async Task LogError_WithUserServiceAccessClaim_ExecutesSuccessfully()
        {
            // Arrange
            var mediatorMock = new Mock<IMediator>();
            ...
            var controller = CreateControllerWithUser(mediatorMock, serviceClaim: "UserService");
        ...
        public async Task LogError_WithAdminRole_ExecutesSuccessfully()
        {
            ...
            var controller = CreateControllerWithUser(mediatorMock, role: "UserServiceAdmin");
            var request = new LogErrorRequest("Service", "Exception", "Message", "Stack", DateTime.UtcNow, "/endpoint", "POST", 500, "user", null);
        ...
        public async Task LogError_WithReadOnlyRole_ExecutesSuccessfully()
        {
            ...
            var controller = CreateControllerWithUser(mediatorMock, role: "UserServiceRead");
            var request = new LogErrorRequest("Service", "Exception", "Message", "Stack", DateTime.UtcNow, "/endpoint", "POST", 500, "user", null);
        ...
        public void Controller_HasCorrectUserContext()
        {
            // Arrange
            var mediatorMock = new Mock<IMediator>();
            var controller = CreateControllerWithUser(mediatorMock, "UserService", "UserServiceAdmin");

            // Assert
            Assert.NotNull(controller.User);
            Assert.True(controller.User.HasClaim("service", "UserService"));
            Assert.True(controller.User.IsInRole("UserServiceAdmin"));
