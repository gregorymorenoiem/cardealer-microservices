...
namespace MessageBusService.Api.Middleware;
    ...
    public async Task InvokeAsync(HttpContext context, IServiceRegistry serviceRegistry, IConfiguration configuration)
    {
        if (!_isRegistered)
        {
            lock (_lock)
            {
                if (!_isRegistered)
                {
                    var serviceName = configuration["Service:Name"] ?? "MessageBusService";
                    var host = configuration["Service:Host"] ?? "localhost";
                    var portString = configuration["Service:Port"] ?? "5009";
                    ...
                    var tags = new[] { "messagebus", "rabbitmq", "transversal", "provider" };
