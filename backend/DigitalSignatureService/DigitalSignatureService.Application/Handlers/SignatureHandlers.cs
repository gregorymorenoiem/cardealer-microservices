...
public class SignDocumentHandler : IRequestHandler<SignDocumentCommand, DigitalSignatureDto>
{
    ...
    public SignDocumentHandler(IDigitalSignatureRepository signatureRepo, IDigitalCertificateRepository certificateRepo)
    {
    ...
    public async Task<DigitalSignatureDto> Handle(SignDocumentCommand request, CancellationToken ct)
    {
        ...
        var signatureValue = GenerateSignature(request.Data.DocumentHash, request.Data.Algorithm);
    ...
    private static string GenerateSignature(string hash, SignatureAlgorithm algorithm)
    {
        // En producción, usar el certificado real para firmar
