        ...
        if (!_isRegistered)
        {
            lock (_lock)
            {
                if (!_isRegistered)
                {
                    try
                    {
                        var serviceName = configuration["Service:Name"] ?? "NotificationService";
                        var host = configuration["Service:Host"] ?? "localhost";
                        var port = int.Parse(configuration["Service:Port"] ?? "45954");
                        ...
                        var instance = new ServiceInstance
                        {
                            ...
                            Tags = new List<string> { "notification", "email", "sms", "push", "provider" },
                        ...
                        logger.LogInformation(
                            "✅ NotificationService registered with Consul: {ServiceName} at {Host}:{Port}",
                    ...
                    {
                        logger.LogError(ex, "❌ Failed to register NotificationService with Consul");
                    }
                }
            }
        }
