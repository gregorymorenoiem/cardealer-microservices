...
// Deletes ALL user data in NotificationService:
//   • Price alerts (búsquedas de precio guardadas)
//   • Saved searches (historial de búsquedas)
//   • In-app notifications (UserNotification)
//
// Queue: notification.user.deletion
// Exchange: cardealer.events (topic)
// Routing key: auth.user.deleted
// DLQ: notification.user.deletion.dlq
// ═══════════════════════════════════════════════════════════════════════════════
...
public sealed class UserDataDeletionConsumer : BackgroundService
{
    ...
    private const string QueueName = "notification.user.deletion";
    ...
    private async Task ProcessUserDeletionAsync(UserDeletedEvent deletionEvent, CancellationToken ct)
    {
        ...
        var userNotificationRepo = scope.ServiceProvider.GetRequiredService<IUserNotificationRepository>();

        var userId = deletionEvent.UserId;

        // Step 1: Delete price alerts (preferencias guardadas)
        var alertsDeleted = await priceAlertRepo.DeleteByUserIdAsync(userId);

        // Step 2: Delete saved searches (historial de búsquedas)
        var searchesDeleted = await savedSearchRepo.DeleteByUserIdAsync(userId);

        // Step 3: Delete in-app notifications
        var notificationsDeleted = await userNotificationRepo.DeleteByUserIdAsync(userId);

        _logger.LogInformation(
            "[UserDeletion] Completed Ley 172-13 cascade for user {UserId}: " +
            "{Alerts} price alerts, {Searches} saved searches, {Notifications} notifications deleted",
            userId, alertsDeleted, searchesDeleted, notificationsDeleted);
    }
    ...
    private void InitializeRabbitMQ()
    {
        try
        {
            ...
            _connection = factory.CreateConnection($"NotificationService-UserDeletion-{Environment.MachineName}");
