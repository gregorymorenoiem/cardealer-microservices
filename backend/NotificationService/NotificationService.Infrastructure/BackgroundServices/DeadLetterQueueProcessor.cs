...
public class DeadLetterQueueProcessor : BackgroundService
{
    ...
    {
        _logger.LogInformation("🚀 NotificationService DLQ Processor started");
        ...
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            {
                _logger.LogError(ex, "❌ NotificationService DLQ Processor error");
    ...
    private async Task ProcessFailedEvents()
    {
        ...
        if (stats.Total > 0)
        {
            _logger.LogInformation("📊 NotificationService DLQ Stats: Total={Total}, Ready={Ready}, Exhausted={Exhausted}",
        ...
        foreach (var failedEvent in eventsToRetry)
        {
            try
            {
                _logger.LogInformation("🔄 NotificationService DLQ retry: {EventType} (Attempt {RetryCount})",
            ...
            {
                _logger.LogError(ex, "❌ NotificationService DLQ retry failed: {EventType}", failedEvent.EventType);
