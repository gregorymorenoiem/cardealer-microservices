...
if (Test-Path "./otel-collector-config.yaml") {
    ...
    try {
        ...
        if ($content -match "receivers:" -and $content -match "exporters:" -and $content -match "service:") {
            Write-Host "[OK] Configuration structure looks valid" -ForegroundColor Green
        }
        else {
            Write-Host "[WARN] Configuration might be incomplete" -ForegroundColor Yellow
        }
    }
    catch {
...
if ($LASTEXITCODE -eq 0) {
    ...
    Write-Host "[WARN] Warning: Some images might not have been pulled" -ForegroundColor Yellow
}
...
if ($LASTEXITCODE -eq 0) {
    Write-Host "[OK] Observability stack started successfully" -ForegroundColor Green
}
else {
...
foreach ($endpoint in $endpoints) {
    try {
        ...
        Write-Host "  [WARN] $($endpoint.Name): Not ready yet (might still be starting)" -ForegroundColor Yellow
    }
}
