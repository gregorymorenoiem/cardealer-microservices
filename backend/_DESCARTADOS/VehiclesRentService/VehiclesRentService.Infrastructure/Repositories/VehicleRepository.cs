...
namespace VehiclesRentService.Infrastructure.Repositories;
...
public class VehicleRepository : IVehicleRepository
{
    ...
    {
        return await _context.Vehicles.AnyAsync(v => v.Id == id && !v.IsDeleted);
    }
    ...
    private IQueryable<Vehicle> BuildSearchQuery(VehicleSearchParameters p)
    {
        ...
        if (p.CategoryId.HasValue) query = query.Where(v => v.CategoryId == p.CategoryId.Value);
        if (p.MinPrice.HasValue) query = query.Where(v => v.Price >= p.MinPrice.Value);
        if (p.MaxPrice.HasValue) query = query.Where(v => v.Price <= p.MaxPrice.Value);
        if (!string.IsNullOrWhiteSpace(p.Make)) query = query.Where(v => v.Make.ToLower() == p.Make.ToLower());
        if (!string.IsNullOrWhiteSpace(p.Model)) query = query.Where(v => v.Model.ToLower() == p.Model.ToLower());
        if (p.MinYear.HasValue) query = query.Where(v => v.Year >= p.MinYear.Value);
        if (p.MaxYear.HasValue) query = query.Where(v => v.Year <= p.MaxYear.Value);
        if (p.MaxMileage.HasValue) query = query.Where(v => v.Mileage <= p.MaxMileage.Value);
        if (p.VehicleType.HasValue) query = query.Where(v => v.VehicleType == p.VehicleType.Value);
        if (p.BodyStyle.HasValue) query = query.Where(v => v.BodyStyle == p.BodyStyle.Value);
        if (p.FuelType.HasValue) query = query.Where(v => v.FuelType == p.FuelType.Value);
        if (p.Transmission.HasValue) query = query.Where(v => v.Transmission == p.Transmission.Value);
        if (p.DriveType.HasValue) query = query.Where(v => v.DriveType == p.DriveType.Value);
        if (p.Condition.HasValue) query = query.Where(v => v.Condition == p.Condition.Value);
        if (!string.IsNullOrWhiteSpace(p.ExteriorColor)) query = query.Where(v => v.ExteriorColor!.ToLower() == p.ExteriorColor.ToLower());
        if (!string.IsNullOrWhiteSpace(p.State)) query = query.Where(v => v.State!.ToLower() == p.State.ToLower());
