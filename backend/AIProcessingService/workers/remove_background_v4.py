...
def remove_bg_local(image_path: Path) -> Image.Image:
    ...
    """
    print("  → Using BRIA RMBG 2.0 (local)...")
    ...
    try:
        ...
        subprocess.run(
            [sys.executable, "-m", "pip", "install", "transformers", "torch", "torchvision", "--quiet"], check=False
        )
        import torch
        from torchvision import transforms
        from transformers import AutoModelForImageSegmentation
...
def remove_bg_rembg_birefnet(image_path: Path) -> Image.Image:
    ...
    """
    print("  → Using rembg BiRefNet (fallback)...")
...
def add_professional_shadow(image: Image.Image) -> Image.Image:
    ...
    """
    print("  → Adding professional shadow...")
    ...
    result.paste(Image.new("RGB", (width, height), (0, 0, 0)), (padding, padding + 3), shadow)
    ...
    try:
        ...
        if mode == "api" and api_key:
            ...
            try:
                ...
                print("  → Falling back to BiRefNet...")
...
def main():
    ...
    parser.add_argument(
        "--api-key",
        type=str,
        default=os.environ.get("REMOVEBG_API_KEY"),
    ...
    if args.mode == "api":
        if not args.api_key:
            ...
            sys.exit(1)
        print("Mode: remove.bg API (professional cloud processing)")
    elif args.mode == "local":
        print("Mode: BRIA RMBG 2.0 (high-quality local processing)")
    else:
        print("Mode: rembg BiRefNet (fallback)")
