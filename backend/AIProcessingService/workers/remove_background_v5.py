...
def install_sam2():
    ...
    subprocess.run([sys.executable, "-m", "pip", "install", "torch", "torchvision", "--quiet"], check=False)
    subprocess.run(
        [
            sys.executable,
            "-m",
...
def remove_bg_sam2(image_path: Path) -> Image.Image:
    ...
    if not sam2_checkpoint.exists():
        print("  → Downloading SAM 2 model (~2.4GB)...")
        sam2_checkpoint.parent.mkdir(parents=True, exist_ok=True)
        import urllib.request

        urllib.request.urlretrieve(
            "https://dl.fbaipublicfiles.com/segment_anything_2/072824/sam2_hiera_large.pt", sam2_checkpoint
        )
...
def remove_bg_rembg_dis(image_path: Path) -> Image.Image:
    ...
    """
    print("  → Using DIS model (high accuracy)...")
...
def remove_bg_combined(image_path: Path) -> Image.Image:
    ...
    """
    print("  → Using combined multi-model approach...")
...
def refine_edges_pro(image: Image.Image) -> Image.Image:
    ...
    """
    print("  → Professional edge refinement...")
