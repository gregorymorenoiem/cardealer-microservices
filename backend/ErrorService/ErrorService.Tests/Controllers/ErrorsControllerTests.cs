...
namespace ErrorService.Tests.Controllers
{
    ...
    {
        private ErrorsController CreateControllerWithUser(Mock<IMediator> mediatorMock, string serviceClaim = "errorservice", string role = "ErrorServiceAdmin")
        {
        ...
        [Fact]
        public async Task LogError_WithErrorServiceAccessClaim_ExecutesSuccessfully()
        {
            // Arrange
            var mediatorMock = new Mock<IMediator>();
            ...
            var controller = CreateControllerWithUser(mediatorMock, serviceClaim: "errorservice");
        ...
        public async Task LogError_WithAdminRole_ExecutesSuccessfully()
        {
            ...
            var controller = CreateControllerWithUser(mediatorMock, role: "ErrorServiceAdmin");
            var request = new LogErrorRequest("Service", "Exception", "Message", "Stack", DateTime.UtcNow, "/endpoint", "POST", 500, "user", null);
        ...
        public async Task LogError_WithReadOnlyRole_ExecutesSuccessfully()
        {
            ...
            var controller = CreateControllerWithUser(mediatorMock, role: "ErrorServiceRead");
            var request = new LogErrorRequest("Service", "Exception", "Message", "Stack", DateTime.UtcNow, "/endpoint", "POST", 500, "user", null);
        ...
        public void Controller_HasCorrectUserContext()
        {
            // Arrange
            var mediatorMock = new Mock<IMediator>();
            var controller = CreateControllerWithUser(mediatorMock, "errorservice", "ErrorServiceAdmin");

            // Assert
            Assert.NotNull(controller.User);
            Assert.True(controller.User.HasClaim("service", "errorservice"));
            Assert.True(controller.User.IsInRole("ErrorServiceAdmin"));
