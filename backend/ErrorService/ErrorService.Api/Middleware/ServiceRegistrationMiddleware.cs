        ...
        if (!_isRegistered)
        {
            lock (_lock)
            {
                if (!_isRegistered)
                {
                    try
                    {
                        var serviceName = configuration["Service:Name"] ?? "ErrorService";
                        var host = configuration["Service:Host"] ?? "localhost";
                        var port = int.Parse(configuration["Service:Port"] ?? "45952");
                        ...
                        var instance = new ServiceInstance
                        {
                            ...
                            Tags = new List<string> { "error", "logging", "provider" },
                        ...
                        logger.LogInformation(
                            "✅ ErrorService registered with Consul: {ServiceName} at {Host}:{Port}",
                    ...
                    {
                        logger.LogError(ex, "❌ Failed to register ErrorService with Consul");
                    }
                }
            }
        }
