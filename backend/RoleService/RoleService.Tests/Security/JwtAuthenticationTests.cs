...
namespace RoleService.Tests.Security
{
    public class JwtAuthenticationTests
    {
        ...
        public void GenerateToken_WithValidClaims_ReturnsValidJwtToken()
        {
            ...
            var claims = new[]
            {
                new Claim("service", "RoleService"),
                new Claim(ClaimTypes.Name, "TestUser"),
                new Claim(ClaimTypes.Role, "RoleServiceAdmin")
            };
        ...
        public void ValidateToken_WithValidToken_ReturnsClaimsPrincipal()
        {
            ...
            Assert.True(principal.HasClaim("service", "RoleService"));
        }
        ...
        {
            // Arrange
            var claims = new[] { new Claim("service", "RoleService") };
            var key = new SymmetricSecurityKey(Encoding.UTF8.GetBytes(_validKey));
            var credentials = new SigningCredentials(key, SecurityAlgorithms.HmacSha256);
        ...
        [Fact]
        public void Token_WithRoleServiceAccessClaim_ShouldHaveServiceClaim()
        {
            // Arrange
            var claims = new[]
            {
            ...
            // Assert
            Assert.True(principal.HasClaim("service", "RoleService"));
            Assert.True(principal.IsInRole("RoleServiceAdmin"));
        }
        ...
        public void Token_WithMultipleRoles_ShouldContainAllRoles()
        {
            ...
            var claims = new[]
            {
                new Claim("service", "RoleService"),
                new Claim(ClaimTypes.Role, "RoleServiceAdmin"),
                new Claim(ClaimTypes.Role, "RoleServiceRead")
            };
            ...
            // Assert
            Assert.True(principal.IsInRole("RoleServiceAdmin"));
            Assert.True(principal.IsInRole("RoleServiceRead"));
        }
    }
}
