                ...
                if (!_isRegistered)
                {
                    try
                    {
                        var instance = new ServiceInstance
                        {
                            ...
                            ServiceName = configuration["Service:Name"] ?? "AdminService",
                            ...
                            Tags = new List<string> { "admin", "consumer" }
                        };
                        registry.RegisterServiceAsync(instance).GetAwaiter().GetResult();
                        logger.LogInformation("✅ AdminService registered with Consul");
                        _isRegistered = true;
                    }
                    catch (Exception ex) { logger.LogError(ex, "❌ Failed to register AdminService"); }
                }
            }
        }
        await _next(context);
