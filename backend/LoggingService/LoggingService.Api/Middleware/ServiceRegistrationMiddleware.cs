    ...
    public async Task InvokeAsync(HttpContext context, IServiceRegistry serviceRegistry, IConfiguration configuration)
    {
        if (!_isRegistered)
        {
            lock (_lock)
            {
                if (!_isRegistered)
                {
                    var serviceName = configuration["Service:Name"] ?? "LoggingService";
                    var host = configuration["Service:Host"] ?? "localhost";
                    var portString = configuration["Service:Port"] ?? "5005";
                    ...
                    var tags = new[] { "logging", "serilog", "seq", "transversal", "provider" };
