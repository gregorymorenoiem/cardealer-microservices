...
namespace ConfigurationService.Api.Middleware;
    ...
    public async Task InvokeAsync(HttpContext context, IServiceRegistry serviceRegistry, IConfiguration configuration)
    {
        if (!_isRegistered)
        {
            lock (_lock)
            {
                if (!_isRegistered)
                {
                    var serviceName = configuration["Service:Name"] ?? "ConfigurationService";
                    var host = configuration["Service:Host"] ?? "localhost";
                    var portString = configuration["Service:Port"] ?? "5003";
                    ...
                    var tags = new[] { "configuration", "settings", "transversal", "provider" };
