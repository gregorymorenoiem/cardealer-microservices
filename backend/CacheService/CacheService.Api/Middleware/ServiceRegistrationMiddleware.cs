...
namespace CacheService.Api.Middleware;
    ...
    public async Task InvokeAsync(HttpContext context, IServiceRegistry serviceRegistry, IConfiguration configuration)
    {
        if (!_isRegistered)
        {
            lock (_lock)
            {
                if (!_isRegistered)
                {
                    var serviceName = configuration["Service:Name"] ?? "CacheService";
                    var host = configuration["Service:Host"] ?? "localhost";
                    var portString = configuration["Service:Port"] ?? "5004";
                    ...
                    var tags = new[] { "cache", "redis", "transversal", "provider" };
